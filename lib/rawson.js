...
Rawson.File.prototype.toBitmap = function (ppmdata){
    ...
    console.time('dcrawToBitmap');
    dcraw.run(['-q','0','/' + this.filename]);
    console.timeEnd('dcrawToBitmap');
    var ppmName = this.filename.split('.')[0] + '.ppm';
    if (dcraw.FS.root.contents[ppmName]){
        return this._PNMtoIMG(this.FS.root.contents[ppmName].contents);
    }
}
...
Rawson.File.prototype._PNMtoIMG = function(ppmDataArr){
    //TODO: make this more efficient, we spend 33% of the raw to img conversion here
    ...
    console.log('Sending image to canvas with', width, 'x', height, 'pixels');
