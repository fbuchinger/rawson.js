...
Rawson.Viewer.prototype.rotatePreview = function (direction, degrees){
    var degrees = degrees || this.getTransformParams().degrees || 0;
    degrees = (direction === 'right' ? degrees + 90: degrees -90);
   this.setTransformParams({degrees: degrees});
    this._updateTransform();
}

Rawson.Viewer.prototype._updateTransform = function (){
    var tParams = this.getTransformParams();
    var transformStr = "rotate(" + tParams.degrees + "deg) scale(" + tParams.scale + ")";
    var transformCSS = {   
        "-moz-transform" : transformStr, 
        "-webkit-transform" : transformStr,
        "transform": transformStr
    }
    this.$preview.css(transformCSS);
}

Rawson.Viewer.prototype.getTransformParams = function (){
    var activeImage = this.$preview.data('activeImage');
    if (!!activeImage === false){
        return false;
    }
    return this.$preview.data(activeImage);    
}

Rawson.Viewer.prototype.setTransformParams = function (tParams){
    var activeImage = this.$preview.data('activeImage');
    if (!!activeImage === false){
        return false;
    }
    var oldParams = this.$preview.data(activeImage);
    this.$preview.data(activeImage,jQuery.extend(oldParams,tParams));
}
...
Rawson.Viewer.prototype.showOnCanvas= function (filename) {
    ...
    this.$preview.data('activeImage', filename);
    if (this.$preview.data(filename)){
        this._updateTransform();
    }
    else {
        this.$preview.data(filename, {scale:1, degrees:0});
        this._updateTransform();
    }
}
